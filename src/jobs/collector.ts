...
export async function pushAll(): Promise<void> {
  ...
  for (const { payeeId, asset } of pairs.values()) {
    try {
      ...
      const amount = (await publicClient.readContract({
        address: config.router, abi: routerAbi, functionName: "claimable", args: [payeeId, asset],
      })) as bigint;
      if (amount === 0n) continue;

      await sendAndWait(
        () => walletClient.writeContract({
          address: config.router, abi: routerAbi, functionName: "pushPayout", args: [payeeId, asset, amount],
        }),
